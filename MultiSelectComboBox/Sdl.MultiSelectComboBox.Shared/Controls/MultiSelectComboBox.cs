...
#endif
{
	...
	public class MultiSelectComboBox : Control, IDisposable {
		...
		private void InitializeInternalElements() {
			...
			if (DropdownListBox == null) {
				...
				if (DropdownMenu != null) {
					...
					if (DropdownListBox != null) {
						...
						// Event handlers are registered in the DropdownListBox property setter, no need to add them here
						// Adding the ContainerContentChanging handler here as it's not in the property setter
			...
			if (ItemsSource != null) {
				...
				if (ItemsSource.Count > 0) {
					// Removed: UpdateSelectedItemsContainer no longer updates selection, handled by ToggleItemSelection
				}
...
#if WINUI
			KeyRoutedEventArgs e) {
			System.Diagnostics.Debug.WriteLine($"[WinUI] KeyUp: {e.Key}, IsDropDownOpen: {IsDropDownOpen}");
		...
		private void ToggleItemSelection(object item) {
			System.Diagnostics.Debug.WriteLine($"[WINUI] ToggleItemSelection called for item: {item}");
			if (item == null) {
				System.Diagnostics.Debug.WriteLine($"[WINUI] ToggleItemSelection: item is NULL, returning");
				return;
			}

#if WINUI
			if (item is ICollectionViewGroup) {
				System.Diagnostics.Debug.WriteLine($"[WINUI] ToggleItemSelection: item is ICollectionViewGroup, returning");
				return;
			}
#else
			...
			bool wasSelected = IsSelectedItem(item);
			System.Diagnostics.Debug.WriteLine($"[WINUI] ToggleItemSelection: item was{(wasSelected ? "" : " NOT")} selected, SelectedItemsInternal.Count={SelectedItemsInternal.Count}");
		...
		private void DropdownListBoxPreviewKeyDown(object sender, KeyEventArgs e) {
			System.Diagnostics.Debug.WriteLine($"[WPF] DropdownListBoxPreviewKeyDown Key={e.Key}, SelectedItem={DropdownListBox?.SelectedItem}");

			if (DropdownListBox != null && DropdownListBox.SelectedItem is object item) {
				switch (e.Key) {
					case Key.Space:
						System.Diagnostics.Debug.WriteLine($"[WPF] Space key - calling ToggleItemSelection for {item}");
						ToggleItemSelection(item);
						e.Handled = true;
						break;
					case Key.Return:
						System.Diagnostics.Debug.WriteLine($"[WPF] Enter key - calling ToggleItemSelection for {item}");
						ToggleItemSelection(item);
						IsDropDownOpen = false;
				...
				if ((Keyboard.Modifiers & ModifierKeys.Shift) != 0 && (e.Key == Key.Down || e.Key == Key.Up)) {
					...
					if (originalSource?.DataContext is object comboBoxItem) {
						System.Diagnostics.Debug.WriteLine($"[WPF] Shift+Arrow - toggling item {comboBoxItem}");
						ToggleItemSelection(comboBoxItem);
					}
				}
			}
		}
		...
		private void DropdownListBoxPreviewMouseUp(object sender, MouseButtonEventArgs e) {
			...
			if (DropdownListBox.SelectedItem is object comboBoxItemFrom && (Keyboard.Modifiers & ModifierKeys.Shift) != 0) {
				if (originalSource?.DataContext is object comboBoxItemTo) {
					System.Diagnostics.Debug.WriteLine($"[WPF] Shift-click range selection from {comboBoxItemFrom} to {comboBoxItemTo}");
